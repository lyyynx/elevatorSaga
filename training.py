...
def elevator_fitness(
    ...
    # Ensure dimensions match exactly as specified
    # weights = Weights(
    #     hidden_1_bias=list(solution[:64]),  # Array(64)
    #     hidden_1_to_hidden_2=[
    #         list(solution[i : i + 16]) for i in range(64, 1087, 16)
    #     ],  # Array(64) [ (16) ] -> Total: 1024
    #     hidden_2_bias=list(solution[1088:1104]),  # Array(16)
    #     hidden_2_to_output=[
    #         list(solution[i : i + 8]) for i in range(1104, 1231, 8)
    #     ],  # Array(16) [ (8) ] -> Total: 128
    #     input_to_hidden_1=[
    #         list(solution[i : i + 64]) for i in range(1232, 3217, 64)
    #     ],  # Array(32) [ (64) ] -> Total: 2056
    #     output_bias=list(solution[3288:3296]),  # Array(8)
    # )
    input = 32
    hidden_1 = 64
    hidden_2 = 16
    ...
    # Hidden 1 Bias: 64 elements (1 bias per node in Hidden Layer 1)
    hidden_1_bias=list(solution[:hidden_1]),  # Index 0 to 63 (64 elements)

    # Input to Hidden 1 Weights: 32 arrays, each with 64 elements (total: 32 * 64 = 2048)
    input_to_hidden_1=[
        list(solution[i : i + input]) for i in range(hidden_1, 1 + input * (hidden_1 + 1), input)
    ],  # Starts at index 64, steps by 64, ends before 2112

    # Hidden 2 Bias: 16 elements (1 bias per node in Hidden Layer 2)
    hidden_2_bias=list(solution[input * (hidden_1 + 1) : input * (hidden_1 + 1) + hidden_2]),  # Index 2112 to 2127 (16 elements)

    # Hidden 1 to Hidden 2 Weights: 64 arrays, each with 16 elements (total: 64 * 16 = 1024)
    hidden_1_to_hidden_2=[
        list(solution[i : i + hidden_1]) for i in range(input * (hidden_1 + 1) + hidden_2, input * (hidden_1 + 1) + hidden_2 * hidden_1 + 1, hidden_1)
    ],  # Starts at index 2128, steps by 16, ends before 3152

    # Output Bias: 8 elements (1 bias per node in Output Layer)
    output_bias=list(solution[input * (hidden_1 + 1) + (hidden_2 + 1) * hidden_1: input * (hidden_1 + 1) + (hidden_2 + 1) * hidden_1 + output]),  # Index 3152 to 3159 (8 elements)

    # Hidden 2 to Output Weights: 16 arrays, each with 8 elements (total: 16 * 8 = 128)
    hidden_2_to_output=[
        list(solution[i : i + output]) for i in range(input * (hidden_1 + 1) + (hidden_2 + 1) * hidden_1 + output, input * (hidden_1 + 1) + (hidden_2 + 1) * hidden_1 + output * hidden_1 + 1, output)
    ],  # Starts at index 3160, steps by 8, ends before 3288
)
...
if __name__ == "__main__":
    ...
    elevator_sagas = [ElevatorSaga(level) for level in range(1, 2)]
